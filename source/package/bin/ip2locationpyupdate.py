...
class IP2LOCATIONUPDATEPY(smi.Script):
    def __init__(self):
        super(IP2LOCATIONUPDATEPY).__init__()

    def get_scheme(self):
        scheme = smi.Scheme("ip2locationpyupdate")
        scheme.description = "ip2locationpyupdate"
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
            ...
            '''logger.error(f"password returned {password}")'''
            ...
            database_code = input_items.get("database_code")
            '''"id": username,'''
            data = {
                "token": password,
            ...
            logger.info(f"Requesting Update: {database_code}")
            _, _ = simpleRequest(
                "/services/ip2locationpy_ip2locationpyupdater/update",
                sessionKey=meta_configs["session_key"],
                method="POST",
                postargs=data,
            )
            logger.info(f"Completed Request: {database_code}")
...
if __name__ == "__main__":
    exit_code = IP2LOCATIONUPDATEPY().run(sys.argv)
    sys.exit(exit_code)
